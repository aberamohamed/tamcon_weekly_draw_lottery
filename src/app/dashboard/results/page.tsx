...
export default function ResultsPage() {
  const { data: drawHistoryData, isLoading } = useQuery({
      ...
      {latestDraw && (
        <Card className="border-none shadow-xl bg-gradient-to-br from-zinc-900 to-zinc-800 text-white overflow-hidden">
          ...
          <CardHeader className="relative z-10">
            ...
            <CardTitle className="text-3xl">Weekly Draw #{(latestDraw._id || latestDraw.id || '').slice(0, 8)}</CardTitle>
            <CardDescription className="text-zinc-400">
              {latestDraw.drawDate ? `Drawn on ${format(new Date(latestDraw.drawDate), 'PPPP')}` : 'Draw date unavailable'}
            </CardDescription>
          </CardHeader>
